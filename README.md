...
> 📊 [아키텍처 플로우 & AI 평가 리포트 보기](https://junsu22.github.io/withbuddy-showcase/withbuddy_ai_report.html)
...
[Multi-Agent Orchestrator]  ← LangGraph (Intent 분류)
     │
     ├─ rag / company_info ───► [Hybrid Search]  ← BM25 + Vector (asyncio parallel)
     │                                │
     ...
     │
     ├─ chitchat ──────────────► 일상 대화 처리
     ├─ communication ─────────► 커뮤니케이션 안내
     ├─ preboarding ───────────► 입사 전 안내
     ├─ out_of_scope_internal ──► 사내 범위 외 안내
     ├─ out_of_scope_external ──► 외부 범위 외 안내
     ├─ sensitive ─────────────► 민감 질문 필터
...
**스트리밍과 품질 검증의 충돌**  
SSE 스트리밍은 답변을 실시간으로 한 글자씩 전송하기 때문에, 한 번 나간 토큰은 회수할 수 없습니다. 그런데 해고·임금체불 같은 고위험 질문은 답변 전체를 검증한 뒤에 보내야 했습니다. 스트리밍과 품질 검증이 구조적으로 충돌하는 상황이었습니다.

해결책은 고위험 질문에 한해 스트리밍을 내부적으로 억제하고, 토큰을 전부 누적한 뒤 LLM Judge로 검증을 마친 다음 단번에 전송하는 방식이었습니다. 사용자 입장에서는 응답이 조금 느려지지만, 잘못된 법적 정보가 그대로 출력되는 것보다는 낫다고 판단했습니다.
